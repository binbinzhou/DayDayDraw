 ...
 * 抽象化表格
 */
function abstractTable(currentNum){
  // ---------内容属性
  ...
  this.trunPageCallBack=function(PageNum){}
}
/*
 表格对象模板
 */
      ...
      for(var i=0;i<len;i++){
        ...
        for(var key in tempDaTa[i]){
        	if(key !== "javaafter"){
        		...
        		$td = $("<td class='nonedata' style='display:none'>").html(tempDaTa[i][key]).appendTo($tr);
        	}
        }
      }
...
//          oA.addClass("current").siblings().removeClass("current"); 
    ...
    page_a.unbind("click").bind("click",function(){
      ...
      tempThis.init((tempThis.currentPageNum-1)*tempThis.displayNum,
        tempThis.currentPageNum*tempThis.displayNum,nowNum);
      //回调函数
      return false;
    });
  }
  //初始化也相当于回调
  this.init = function(begin,end,PageNum){
    ...
    this.showFoot();
    //初始化分页回调函数
    this.trunPageCallBack(PageNum);
  }
  this.init(0,options.displayNum,1);
